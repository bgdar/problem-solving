...
void displayArray(int buffer[]) {
  for (int nilai_awal = 0; nilai_awal <= ARRAYSIZE; nilai_awal++) {
    // pemisah " " di akhir agar terpisah dari sampah panjang array yang belum
    // di isi
...
int main() {
  ...
  std::cout << "array yang di cari ada di index :"
            << searchArrayValue(buffer, 20) << std::endl;

  std::cout << "last value :" << getLastValue(buffer, panjang_array)
            << std::endl;
